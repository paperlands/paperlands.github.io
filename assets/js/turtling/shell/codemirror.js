...
function findBlockEnd(cm, start) {
  let currentLine = start.line;
  let startIndent = cm.getLine(currentLine).search(/\S/);
  let depth = 1;

  for (let i = currentLine + 1; i <= cm.lastLine(); i++) {
    ...
    // When depth reaches 0, we've found the matching end block
    if (depth === 0) {
      return {

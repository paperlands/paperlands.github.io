(function(){
  ...
  function fbm(x,y){
    return vnoise(x,y)*0.6 + vnoise(x*2.1,y*2.1)*0.28 + vnoise(x*4.3,y*4.3)*0.12;
  }
  ...
  function terrainY(c, r, t){
    /* world coords */
    var nx = c*0.16, ny = r*0.22 + t*0.05;
    var h = fbm(nx, ny);
    /* ridge accent */
  ...
  function drawTerrain(t){
    ...
    for(c=0; c<=COLS; c+=4){
      ...
      for(r=0; r<ROWS; r++){
        ...
        if(r===0) ctx.moveTo(x2,y2); else ctx.lineTo(x2,y2);
      }
      ctx.stroke();
    }
    /* horizon glow */
  ...
  (function(){
    for(var i=0;i<NFLIES;i++){
      flies.push({
        x: Math.random(), y: 0.25 + Math.random()*0.7,
  ...
  function drawFlies(t){
    ...
    for(var i=0;i<flies.length;i++){
      ...
      g.addColorStop(0.4,'rgba('+PHOS+','+(0.22*blink).toFixed(3)+')');
  ...
  function makeSpriteML(linesArr, px, color, glowAlpha){
    ...
    cv.width = w*SPR; cv.height = h*SPR;
  ...
  var SIZE_A = 9, SIZE_B = 8;

  function pathPoint(tt){
    return {
      x: ((tt*DRIFT) % 1.2) - 0.1,
      y: 0.87 + 0.030*Math.sin(tt*0.23) + 0.012*Math.sin(tt*0.61),
      z: Math.sin(tt*0.17)                /* -1 far .. +1 near */
    };
  }
  function persp(z){ return lerp(0.78, 1.22, (z+1)/2); }
  function depthAlpha(z){ return lerp(0.6, 1, (z+1)/2); }

  var fellowship = {
    flapSp: 2.6,
    framesA: null, framesB: null,
    trailA: [], trailB: [], trailT: 0,
    lastAx: -1, lastBx: -1
  };
  var TRAIL_LEN = 7;
  ...
  function buildSprites(){
    ...
    fellowship.framesB = [];
    for(var f=0; f<3; f++){
      fellowship.framesA.push(makeSpriteML(FRAMES_A[f], SIZE_A, 'rgba('+EMBER+',0.55)', '0.5'));
      fellowship.framesB.push(makeSpriteML(FRAMES_B[f], SIZE_B, 'rgba('+EMBER+',0.45)', '0.4'));
    }
  }
  buildSprites();
  ...
  function drawFellowship(t, dt){
    ...
    var pA = persp(A.z), pB = persp(B.z);
    var aA = depthAlpha(A.z), aB = depthAlpha(B.z);
    var ax = A.x*W, ay = A.y*H - A.z*5;   /* near drifts a touch lower */
    var bx = B.x*W, by = B.y*H - B.z*5;
    ...
    function drawTrail(arr, base){
      ...
      for(var k=0;k<n;k++){
        ...
        ctx.fillStyle = 'rgba('+EMBER+','+(base*fade*fade).toFixed(3)+')';
        ctx.fillRect(arr[k*3]*W - s/2, arr[k*3+1]*H - arr[k*3+2]*5 - s/2, s, s);
      }
    }
    drawTrail(p.trailA, 0.10);
    ...
    function blit(sp, x, y, sc, alpha){
      ...
      ctx.drawImage(sp.cv, x - sp.w*sc/2, y - sp.h*sc/2, sp.w*sc, sp.h*sc);
    }
    if(farFirst){
    ...
    var vy = 0.030*0.23*Math.cos(t*0.23) + 0.012*0.61*Math.cos(t*0.61);
    lastHeadingDeg = Math.round(((Math.atan2(vy, DRIFT)*180/Math.PI)%360+360)%360);
  }
  /* ================= FIVE WORLD WINDOWS =================
  ...
  function worldGemstone(c,w,h,t){
    c.clearRect(0,0,w,h);
    var cx=w/2, cy=h/2, n=6, layers=9;
    var glintA = t*0.5;
    for(var k=0;k<layers;k++){
      var rr = Math.min(w,h)*0.42 * (1 - k/layers);
      var rot = k*0.35 + t*0.05*(k%2? 1:-1);
      var base = lerp(0.40, 0.08, k/layers);
      for(var i=0;i<n;i++){
  ...
  function worldArcs(c,w,h,t){
    c.clearRect(0,0,w,h);
    var cx=w*0.5, cy=h*0.95;
    for(var k=0;k<8;k++){
      var rr = lerp(h*0.15, h*0.95, k/7) * (1 + 0.03*Math.sin(t*0.7 + k));
      var sweep = lerp(0.4, 1, hash2(k,7)) * Math.PI * (0.7 + 0.3*Math.sin(t*0.5 + k*1.7));
      var start = Math.PI + (Math.PI - sweep)/2 + Math.sin(t*0.2 + k)*0.25;
      c.strokeStyle='rgba('+PHOS+','+lerp(0.4,0.1,k/7).toFixed(3)+')';
      c.lineWidth = 1;
      if(k%3===2) c.setLineDash([3,5]); else c.setLineDash([]);
      c.beginPath();
  ...
  function worldRecursion(c,w,h,t){
    ...
    function branch(x,y,len,ang,depth){
      ...
      c.strokeStyle='rgba('+PHOS+','+lerp(0.08,0.45,depth/6).toFixed(3)+')';
  ...
  function worldHeartbeat(c,w,h,t){
    ...
    c.lineWidth=1;
    for(var gx=0; gx<=w; gx+=w/12){ c.beginPath(); c.moveTo(gx,0); c.lineTo(gx,h); c.stroke(); }
    for(var gy=0; gy<=h; gy+=h/6){ c.beginPath(); c.moveTo(0,gy); c.lineTo(w,gy); c.stroke(); }
    var phase = t*0.35;
    var beatAtCenter = Math.exp(-Math.pow((((0.5+phase)*3)%1-0.25)*9,2));
    c.strokeStyle='rgba('+PHOS+','+(0.4 + 0.35*beatAtCenter).toFixed(3)+')';
  ...
  function worldFlock(c,w,h,t,dt){
    ...
    for(i=0;i<boids.length;i++){
      var b = boids[i];
      var ax=0, ay=0, cxn=0, cyn=0, sx=0, sy=0, n=0;
      for(j=0;j<boids.length;j++){
        ...
        if(d2 < 60*60){
          ...
          cxn+=o.x; cyn+=o.y;          /* cohesion  */
          if(d2 < 18*18 && d2>0.01){ sx-=dx/d2*220; sy-=dy/d2*220; } /* separation */
        }
      }
      if(n>0){
  ...
  var WORLD_DRAWS = {1: worldGemstone, 2: worldArcs, 3: worldRecursion,
                     4: worldHeartbeat, 5: worldFlock};
  var panes = [];
  ...
  function setupPanes(){
    ...
    for(var i=0;i<cs.length;i++){
      ...
      panes.push({ el:cs[i], c:s.c, w:s.w, h:s.h, draw:draw, visible:true });
    }
  ...
  document.addEventListener('visibilitychange', function(){
    ...
    else if(!reduceMotion){ running = true; prev = performance.now(); requestAnimationFrame(frame); }
  });
  ...
  if(pano){
    ...
    function renderPano(){
      ...
      pano.width = Math.floor(pw*DPR); pano.height = Math.floor(ph*DPR);
